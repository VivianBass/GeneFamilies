...
message("<mcl_output.txt> a TAB separated textual output generated by mcl Tool (markov clustering), 
        containing gene-family clusters and the respective gene members associated with the cluster")
...
families.df <- read.table(input.args[[2]], sep = "\t", header = TRUE, stringsAsFactors = FALSE,
    comment.char = "", quote = "", na.strings = "", colClasses = c("character", rep("numeric", 1)))
families.df$size <- families.df$dmel

# Save data:
save(families.genes.df, families.lst, families.df, file = file.path(output_data_dir, "families.RData"))
