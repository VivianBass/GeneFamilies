...
message("USAGE: Rscript exec/generate_t-test_wilcox_test_tissue_log2.R")
...
load(file.path(output_data_dir, "exp.prof.dists_mean_median_tissue_log2.RData"))
...
test_results_tissue <- tryCatch({
    ...
    if (!is.null(median_results) && !is.null(mean_results)) {
        ...
        write.csv(test_summary_tissue, 
                  file.path(results_dir, "statistical_tests_summary_tissue_log2.csv"), 
                  row.names = FALSE)
        message("Tissue-specific statistical tests summary (log2) exported to CSV")
    }
