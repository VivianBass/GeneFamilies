...
# Select log2 transformed data objects
data_names <- loaded_objects[grepl(".lst_dists_log2$", loaded_objects)]
data_names_tissue <- loaded_objects[grepl(".lst_dists_tissue_log2$", loaded_objects)]

# Process regular log2 distances
df_complete_dists <- data_names %>%
    map_df(function(data_name) {
        current_data <- get(data_name)
        type_name <- sub(".lst_dists_log2", "", data_name)
...
# Process tissue-specific log2 distances
df_complete_dists_tissue <- data_names_tissue %>%
    map_df(function(data_name) {
        current_data <- get(data_name)
        type_name <- sub(".lst_dists_tissue_log2", "", data_name)
...
output_pdf <- file.path(results_dir, "complete_boxplots_expression_distances_log2_(t.test_wilcox).pdf")
...
# Save t-test plots
output_pdf_tea <- file.path(results_dir, "complete_boxplots_tissues_expression_distances_log2_(t.test).pdf")
ggsave(output_pdf_tea, marrangeGrob(plot_list_tea, nrow=1, ncol=1, top=""), 
       width = 12, height = 8)

# Save wilcoxon test plots
output_pdf_wilcox <- file.path(results_dir, "complete_boxplots_tissues_expression_distances_log2_(wilcox.test).pdf")
