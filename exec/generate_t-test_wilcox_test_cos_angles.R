...
message("USAGE: Rscript exec/generate_t-test_wilcox_test_angles.R")
...
load(file.path(output_data_dir, "exp.prof.angles_mean_median.RData"))
...
test_results <- tryCatch({
    ...
    if (!is.null(median_results) && !is.null(mean_results)) {
        ...
        write.csv(test_summary, 
                 file.path(results_dir, "statistical_tests_summary_angles.csv"), 
                 row.names = FALSE)
        message("Statistical tests summary for angles exported to CSV")
    }
