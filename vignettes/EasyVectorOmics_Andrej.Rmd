...
- (1) `orthologsTandems.RData` 

  [1] "orthologs" "tandems"			

- (2) `GeneGroups.RData` 				 

  [1] "fubar.fams.decisive.evidence.genes" 	    "fubar.fams.decisive.evidence.genes.nev"                                                                   
...
- (3) `families.RData`       
  ...
  [1] "families.df" "families.genes.df" "families.lst"
...
-----------------------------------------------------------------------------------------
  ...
  - computeExpressionProfileDistances.R                             (from Section-2)

  - (1) `investigateDistributionsOfExpressionProfileDistances.R`		(Step-1)
                                
  - (10) `parseCafeResult.R`                                        (used in Sidestep-DefineGeneSets)

  - (11) `exec/loadBustedResults.R`                                 (used in Sidestep-DefineGeneSets)
...
-----------------------------------------------------------------------------------------
  ...
  - `families.exp.prof.dists` 			    (from Section-2 - Step-4 / as input (1)) 
  - `families.exp.prof.dists.tissue`		(from Section-2 - Step-4) 
  - `orthologs.exp.prof.dists` 			    (from Section-2 - Step-4 / as input (1)) 
  - `orthologs.exp.prof.dists.tissue`		(from Section-2 - Step-4)
  - `tandems.exp.prof.dists` 				    (from Section-2 - Step-4 / as input (1)) 
  - `tandems.exp.prof.dists.tissue`		  (from Section-2 - Step-4)
...
- `ExpressionProfileDistances.RData` 	(from Section-2 - Step-4) 
...
-----------------------------------------------------------------------------------------
  ...
  - `cosDiag()`							                (from expression_funks.R)  
...
-----------------------------------------------------------------------------------------
  ...
  - 2. `meanTissueVersatilityDiffsAfterDuplicationHistograms.pdf`		((2) generated)
  - 3. `afterDuplicationAngleBetweenOrth2DiagVecsBoxplot.pdf`			  ((2) generated)
  - 4. `afterDuplicationAngleBetweenOrth2DiagVecsHistograms.pdf`		((2) generated)
  - 5. `meanChangeInAngleBetweenOrthOnDiagsBoxplot.pdf`				      ((2) generated)

  - `absLog2FoldChangeIntraSpeciesBoxplots.pdf`					            ((2) generated)(missing)
...
- The dataset includes 5 different tissues or conditions: 
  cotyledon, developing leaf, seedling, flower stage 16, and flower stage 9.
...
- Each gene is assigned a count number representing its expression activity, 
  which indicates whether the gene is active or inactive in a given tissue (ON/OFF/Repressed).
...
- This data shows the expression (count value) for each gene of 
  Cardamine hirsuta in various tissues (locations/time conditions).
- The expression data for 56,874 genes in Cardamine hirsuta is assigned 
  an expression value (gene activity) based on the tissue in which they are expressed 
...
-----------------------------------------------------------------------------------------
  ...
  - `mcl_output.txt` , `mcl_table.tsv` (from markov clustering tool mcl)  (fehlt) 
...
- In Step 4, Euclidean distances are calculated within a specific gene cluster. 
  This process involves comparing the expression profiles of each gene pairwise, 
  effectively measuring the similarity or difference in their activity levels. 
  The dist function in R is used for this calculation, where it computes the pairwise distances 
  between the rows of a matrix representing gene expression profiles. 
  The result is stored in a dist object, which contains the computed distances in a compact form.
...
- 'dist' num is a distance object storing pairwise Euclidean distances 
   between the gene expression profiles within the cluster. The values represent the 
   Euclidean distances between pairs of genes within the cluster.

- 'dist' num [1:12880] 0.61 0.962 0.436 0.681 0.336 ...:

- Each distance object (dist) within a cluster is a condensed form of a symmetric matrix 
  that stores the Euclidean distances between all pairs of genes in that cluster. 
  (All vs All comparison within a Cluster)
- [1:12880] indicates that there are 12,880 pairwise distances calculated for cluster_1. 
- The distances are stored in a single vector, where each element corresponds 
  to the distance between a unique pair of genes.

- For example: the first value in the vector might represent the distance between the 
  first and second genes in the cluster, the second value might represent the    
  distance between the first and third genes, and so on.
...
- Like the above `families.exp.prof.dists.tissue` but for genes belonging to tandem clusters.
...
-----------------------------------------------------------------------------------------
  ...
  - also summarizes the different distances per gene group using various statistics, 
    like `mean`, `median`, `max`, and `maxMinusMin`:

  - Finally carry out the mentioned t-tests and generate the corresponding plots
...
- Class-Specific Distances: The script separates expression profile distances 
  into two categories—Orthologs and Non-Orthologs—using predefined gene classes, 
  and calculates statistics (e.g., max, median) for each class within the clusters.

- There are three main lists—orthologs, tandems, and families—representing different gene groups.

- Each list contains clusters, which are objects that hold 
  groups of genes belonging to a specific gene family. 
  The tandems list includes clusters for tandem gene families, 
  while the orthologs list contains clusters for orthologous gene families.

- in each Cluster and in each list the euclidean distances were already calculated 

- in each Cluster and in each list the euclidean distances were already calculated 
-> so all genes (their gene Expression Values) in that Cluster 
  were paired against each in that Cluster (All vs All) 
- euclidean dist numbers --> representing the changes between the paired gene Expressions. 
  and Therefor Changes in the Gene Activity
...
--> stored in `ExpressionProfileDistanceDistributions.RData` 
  ...
  - The following dataframes represent statistical analyses of gene family clusters. 
    For each cluster listed in orthologs.exp.prof.dists, families.exp.prof.dists, 
    and tandems.exp.prof.dists, statistical evaluations have been conducted, 
    calculating the maximum, median, mean, and max-minus-min values of the Euclidean distances. 
    This results in a dataframe where each cluster is represented four times in a row, 
    with these respective statistical values listed for each entry.
...
```
- tandems.exp.prof.dists.orth.dist: 
  A list containing 3,266 elements (one for each tandem cluster). 
  Each element is a list with expression profile distances 
  for Orthologs, Non-Orthologs, and a summary statistics data frame.

- tandems.exp.prof.dists.orth.dist.df: 
  A data frame combining the statistics for all tandem clusters, 
  with columns for Family, Statistic, Gene Class, and Value. 
  It has 26,128 rows because statistics are computed for both Orthologs and Non-Orthologs 
  across multiple tandem clusters.
...
```
- Families: Similar processing is applied to gene families (families.exp.prof.dists), 
  where class-specific expression profile distances are computed, 
  and the resulting statistics are combined into families.exp.prof.dists.orth.dist.df.
...
```
- Orthologs: The script also processes orthologous gene clusters (orthologs.exp.prof.dists) 
  to calculate and store statistics, which are then combined into orthologs.exp.prof.dists.stats.df.
...
- p.df (Plot-Data-1) is used to generate the following plots:
...
- p.df (Plot-Data-2) is used to generate the following plots:
...
- 3. `medianExpressionProfileDistancesPerGeneGroupClassesBoxplot.pdf`     
  ...
  - This plot includes an additional group, "PSel,"
  - without comparing "Orthologs" and "Non-Orthologs. 

  - Plot-Type: 		          Boxplot

  --> x-Achsis: 				    (Orth, Tand, Exp, PSel)
  --> y-Achsis Skala: 	    median, euclidean Distances, between expression Profiles

  - median, euclidean Distances, between expression Profiles
  -> 
...
-  4. `medianExpressionProfileDistancesPerGeneGroupClassesBoxplot_NoPosSel.pdf`  
  ...
  - This plot includes no "PSel," group
  - without comparing "Orthologs" and "Non-Orthologs. 
...
- 5. `medianExpressionProfileDistancesPerGeneGroupClassesAndOrthologDistinctionBoxplot.pdf`   
  ...
  -> indicating that even though positive selection often leads to divergence, 
    the overall expression profiles may still retain some degree of similarity.
...
- 6. `medianExpressionProfileDistancesPerGeneGroupClassesAndOrthologDistinctionBoxplot_NoPosSel.pdf`		  
  ...
  - Non-Orthologs have higher median expression distances, showing more variability 
    in expression profiles within both gene groups.
  - generally exhibit greater variability and higher median distances in expression profiles 
    across all gene groups ("Tand," "Exp," and "PSel"), 
    indicating more diversity in how these genes are expressed.
...
- p.df (Plot-Data-3) is used to generate the following plots:
...
- Data-Frames used as Input to generate p.df (Plot-Data-1):
-----------------------------------------------------------------------------------------
...
- Code Section from `exec/expression.Angles.R` used to generate p.df 
...
```
  ...
  - mean.base.tiss.vers:          -> Mean tissue versatility of the gene family 
                                     before any duplication or positive selection.
  - tandem.tiss.vers:             ->
  - tandem.tiss.change:           ->
...
```
  ...
  - duplicated.dist.vec.clouds & pos.selected.dist.vec.clouds: 
    Measures of how expression profiles are distributed in the vector space, 
    providing insights into how expression patterns change after duplication or under positive selection.
...
- 3. `meanTissueVersatilityDiffsAfterDuplicationBoxplot.pdf`         
  ...
  - mean difference:      The average of a set of data points, 
  - Tissue-Versatility:   Measures how broadly a gene’s expression is distributed across different tissue types.
                          A higher tissue versatility indicates that the gene's expression 
                          is more widely distributed across different tissues.
                          while a gene with low tissue versatility is expressed in only a few.
  - after duplication:  

  --> duplicated genes show little to no change in tissue versatility after duplication
  --> tandem genes show a small positive change in tissue versatility after duplication
...
- 4. `meanTissueVersatilityDiffsAfterDuplicationHistograms.pdf`           
  ...
  - duplicated Genes: most duplicated genes do not show a significant change in tissue versatility, 
    with some exceptions in both directions (positive and negative).

  - Tandem Genes: The distribution is more spread out compared 
    to the duplicated genes and slightly skewed to the right, 
    indicating that while many tandem genes do not change much in tissue versatility, 
    there are a larger number that do show an increase, and the increase can be more substantial.
...
- Duplicated Genes: The relatively moderate angles suggest that while 
  duplicated genes do show some divergence in expression patterns, 
  this divergence is not extreme. This might imply that duplicated genes 
  often retain some degree of functional similarity.

- Tandem Genes: The slightly higher median and broader spread 
  in angles for tandem genes indicate that these 
  gene duplicates are more likely to show significant divergence in their expression patterns. 
  This could reflect a greater potential for functional diversification among tandem duplicates, 
  possibly due to the influence of their proximity on the chromosome, 
  leading to different regulatory environments.

- Conclusion:
  This boxplot suggests that tandem duplicates might be 
  subject to higher evolutionary pressures that 
  lead to greater divergence in gene expression patterns compared to other duplicated genes. 
  This could be significant for understanding how tandem duplicates contribute 
  to functional innovation and diversity in the genome.
