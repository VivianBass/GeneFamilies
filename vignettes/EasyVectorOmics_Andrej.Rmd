...
They typically retain the same function across species.

			
- 2. Tandem Duplicates, tandems (Tand) Gene-Group: 
...
--> Expanded gene families (Clusters) are groups of genes that have undergone significant expansion in a particular lineage, 
often through duplication events. (expanded Gene-Family (Cluster) --> contain a larger number of similar genes from one or more species)

- 4. and Gene Families showing signs of positive selection (PSel), Gene-Group: 
--> Positive selection refers to evolutionary pressure that favors advantageous mutations. 
These clusters may include orthologs that have diverged significantly due to selective pressures.
...
- a cluster refers to a group of genes that are related by sequence similarity and are assumed to have evolved from a common ancestor.
- each Gene-Group contains a List of Clusters, with Genes like Orthologs, Tandems, etc. beeing assigned to the respective Clusters. 

->  These clusters are identified by analyzing the sequences of genes across multiple species, 
finding similarities, and grouping them based on these similarities.


- Clustering, Groupping Process -> Genes assigned to the Clusters
- using (Blast, Markov) 
- 8 Brassicacean Genoms
- On all coding sequences available for the eight genomes ran an all-vs-all BLAT (Blast)
- using reciprocal best hits in an all-vs-all Sequence-Similarity-Search
- Using this the results were fed into Markov Clustering based on Pairwise Sequence Similarities in percent.
- The MCL algorithm is implemented in the popular `mcl` program (`mcl 14-137, van Dongen`)
> It's output has been read in by the following script.
...
> The Perl-Script `mcl2orthoformat.pl` can be used to convert the MCL output into the table.
...
--> we meassure Differences in gene expressions bzw. Gene expression profiles
--> Distances are computed between whole Gene expression profiles as well as on each euclidean dimension (tissue).
--> gene expression changes are measured in distances between mean expression vectors


- Differences in gene expression are correlated with function diversification.
- expression vector space / gene expression changes due to influeces like
- belonging to a certain gene group / gene family / orthologs / paralogs / or tandem duplicates
- gene expression changes are measured in distances between mean expression vectors
...
# RScripts and Files Overview -------------------------------------------------------------------

- Rscripts 
- rpkm.rna.seq.counts.R
...
[28] "fubar.fams.decisive.evidence.genes"     "fubar.fams.decisive.evidence.genes.nev" "genes.pos.sel"
[31] "genes.pos.sel.nev"                      		                                "orthologs.genes"
[34] "orthologs.lst"                          "orths.expr"                             "psel.classifier"
[37] "psel.expr"                              "psel.expr.nev"                          "psel.w.orths"
[40] "tand.classifier"                        "tand.psel.classifier"                   
[43] "tandems.genes"                          "tandems.genes.nev"                      "tandems.lst"
[46] "tands.expr"                             "tands.expr.nev"                         "tands.w.orths"
...
- absLog2FoldChangeIntraSpeciesBoxplots.pdf					((2) generated)
...
- Inputs expressionAngles.R:
 
- rpkm.expr.profiles.df
- tands.expr , orths.expr , dupl.expr , psel.expr (GeneSets)
...
'data.frame':   246 obs. of  8 variables:
 ...
 r$> summary(dupl.psel.w.orths.angles.df) -> Family Length: 246, Class :character
 
   mean.base.tiss.vers duplicated.tiss.vers duplicated.tiss.change duplicated.dist.vec.clouds pos.selected.tiss.vers pos.selected.tiss.change    
   Min.   :0.2941      Min.   :0.2980       Min.   :  0.7317       Min.   :-1476.532          Min.   :0.3013         Min.   :  5.415
   1st Qu.:0.3242      1st Qu.:0.3304       1st Qu.: 31.5348       1st Qu.:  -48.831          1st Qu.:0.3372         1st Qu.: 36.895
   Median :0.3595      Median :0.3854       Median : 52.9793       Median :   -1.179          Median :0.3718         Median : 62.206
...
- dupl.w.orths.angles.df
- dupl.psel.w.orths.angles.df
- require: fruit.ath.intra, fruit.chi.intra, light.ath.intra, light.chi.intra: (fehlen)
...
--------------------------------------------------------------------------------------------------
Plot Descriptions - for Section-3
--------------------------------------------------------------------------------------------------
...
--> x-Achsis basically representing the Gene-Group (Class)
--> y-Achsis basically plots the euclidean Distances of Gene Expression Profiles

Mean: The average of a set of data points, calculated by dividing the sum of all points by the number of points.

Median: The middle value of a data set when ordered, not necessarily the average.

Euclidean Distance: Measures the "straight-line" distance between two expression profiles in multi-dimensional space.

Median Euclidean Distance: The middle value of all pairwise distances within a cluster, showing typical expression profile differences.

Median Euclidean Distance: For each cluster, this value represents the typical change in gene expression between pairs of genes. It was used as a data point in the analysis.
Data Points: Each data point corresponds to the number of clusters within each gene group.


Gene Groups Used for the Plots:

Orthologs: Genes in different species with a shared ancestral origin, typically retaining similar functions.
Tandems: Genes located close to each other on the same chromosome, often sharing regulatory mechanisms.
Exp: (Details not provided)
PSel: (Details not provided)


- Orthologs & Non Orthologs

Orthologs: Genes from different species with a shared ancestor, generally having similar functions and lower median Euclidean distances (more similar expression profiles).
Non-Orthologs: Genes without a shared ancestor, including paralogs (within species) and analogous genes (different evolutionary paths). They show higher median Euclidean distances (more varied expression profiles).


The box plots shown compare the median Euclidean distances between expression profiles 
...
--> are without the comparison between Orthologs and Non Orthologs
...
--> also include the comparison between Orthologs and Non Orthologs
...
--------------------------------------------------------------------------------------------------
Plot Descriptions - for Section-4
--------------------------------------------------------------------------------------------------
...
--> Data-Frames used to create the Plots
