...
def add_connections_as_individual_entries_majority_rule(left_leafs, right_leafs, scoring_matrix):
    """
    Matches species between the left and right leaf lists based on single-species rules
    or a scoring matrix to find the best pair for multiple species, assigning all pairs
    to the gene with the highest number of connections.

    Args:
        left_leafs (List[Clade]): Leaf nodes from the left subtree.
        right_leafs (List[Clade]): Leaf nodes from the right subtree.
        scoring_matrix (Dict[Tuple[str, str], float]): Precomputed scores for leaf pairs.

    Returns:
        List[Tuple[str, str]]: Matched pairs of leaf node names.
    """
    connections = []
    left_species_map = defaultdict(list)
    right_species_map = defaultdict(list)

    # Map leaves to their species
    for left_leaf in left_leafs:
        species = get_species_name(left_leaf.name)
        left_species_map[species].append(left_leaf)

    for right_leaf in right_leafs:
        species = get_species_name(right_leaf.name)
        right_species_map[species].append(right_leaf)

    # Match leaves based on species
        ...
        for right_species, right_clade_list in right_species_map.items():
            if left_species != right_species:
                if len(left_clade_list) == 1 and len(right_clade_list) == 1:
                    ...
                    # Count best pairs for each left gene
                    pair_counts = defaultdict(int)
                    best_pairs = {}  # Store the best pair for each right gene
                    ...
                    for left_clade in left_clade_list:
                        for right_clade in right_clade_list:
                            ...
                            current_best = best_pairs.get(right_clade.name)

                            if not current_best or pair_score > current_best[1]:
                                # Update the best pair for the right gene
                                best_pairs[right_clade.name] = (left_clade.name, pair_score)

                    # Count the number of pairs assigned to each left gene
                    for pair in best_pairs.values():
                        pair_counts[pair[0]] += 1

                    # Identify the left gene with the highest pair count
                    max_gene = max(pair_counts, key=pair_counts.get, default=None)

                    if max_gene:
                        # Assign all pairs to the max_gene
                        for right_gene, (best_left_gene, _) in best_pairs.items():
                            connections.append((max_gene, right_gene))
...
def add_connections_as_individual_entries(left_leafs, right_leafs, scoring_matrix):
    ...
    for left_species, left_clade_list in left_species_map.items():
        for right_species, right_clade_list in right_species_map.items():
            if len(left_clade_list) == 1 and len(right_clade_list) == 1 and left_species != right_species:
                connections.append((left_clade_list[0].name, right_clade_list[0].name))
            else:
                best_pair = None
                ...
                for left_clade in left_clade_list:
                    for right_clade in right_clade_list:
                        pair_score = calculate(left_clade, right_clade, scoring_matrix)
                        if pair_score > best_score and left_species != right_species:
...
def process_tree(tree, orthogroup, scoring_matrix,duplication_dict):
    ...
    def traverse(clade):
        ...
        # print(left_leafs, orthogroup)
        # print()
        right_leafs = get_all_leafs(clade.clades[1]) if len(clade.clades) > 1 else []

        if left_leafs and right_leafs:
            connections = add_connections_as_individual_entries_majority_rule(left_leafs, right_leafs, scoring_matrix)
...
def main():
    ...
    pickle_file = "harmonic_means_by_prefix.pkl"
    Duplications_file = r"OrthoFinder/Results_Nov22/Gene_Duplication_Events/Duplications.tsv"
    tree_files = r"OrthoFinder/Results_Nov22/Resolved_Gene_Trees"
    df_orthogroups = pd.read_csv('OrthoFinder/Results_Nov22/Orthogroups/Orthogroups.tsv', sep='\t')
    ...
    co_path = f"results/complete_tree_conserved_orthologs_{th}.tsv"
    in_path = f"results/complete_tree_in_paralogs_{th}.tsv"
    sin_path = f"results/complete_tree_special_in_paralogs_{th}.tsv"
    sout_path = f"results/complete_tree_special_out_paralogs_{th}.tsv"
    out_path = f"results/complete_tree_out_paralogs_{th}.tsv"
    ...
    for filename in sorted_files:
        ...
        result_conserved_orthologs = process_tree(tree, orthogroup, harmonic_dict, Duplications_df)
