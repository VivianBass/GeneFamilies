...
#data/blast/all_vs_all_results.txt
#data/blast/all_vs_all.tsv
#data/orthologs/Orthogroups.txt
#data/paralogs/paralogs.tsv

input.args[[1]] <- "data/blast/all_vs_all_results.txt"
input.args[[2]] <- "data/orthologs/mapped_orthogroups.tsv"
input.args[[3]] <- "data/paralogs/paralogs.tsv"
...
orths.nms <- paste("ortholog_cluster_", 1:nrow(df.1hit.ortho), sep = "")
orthologs.lst <- setNames(mclapply(1:nrow(df.1hit.ortho), function(x) unlist(df.1hit.ortho[x, 
    ])), orths.nms)
orthologs.genes <- unlist(orthologs.lst)
...
save(paralogs, paralogs.lst, file = file.path("data", "paralogs.RData"))
...
# loadOrthologsAndParalogs.R

"x/data/orthologs/dmel_drosophila_species_orthologs_2024_filtered.tsv"
...
# colClasses = rep("character", 3) --> Anzahl bearbeiten
